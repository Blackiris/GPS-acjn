...
/**
 * Classe qui fait le lien au niveau des données entre le bean et l'interface graphique
 * 
...
public class DataModel {
	...
	public void addNote(Note newNote) {
		...
		//appel de la fonction addNote de AdminBeanRemote qui effectue l'ajout dans la base de données
	...
	/**
	 * Méthode qui récupère une note depuis la base de donnée en se basant sur ses coordonées
	 * @param coor coordonées de la note à récupérer
	 * @return Note
	 */
	 ...
	 * @param coor coordonnées de la note à mettre à jour
	 * @param note mise à jour pour l'ancienne note
	 */
	public void updateNote(SCoordinate coor, Note note) {
		...
		//on appelle la méthode de AdminBeanRemote qui se charge de mettre à jour la note concernée dans la base de données
		ClientAdmin.adminBeanRemote.updateNote(coor, note);
		//on met à jour l'itinéraire de la liste des itinéaires dont la note a changé
		updateRelatedItinerary(coor, note);
	}

	/**
	 * méthode qui supprime une note de la Map et de la base de données
	 * @param coor coordonées de la note en question
	 */
	public void removeNote(SCoordinate coor) {
		//retire la note de la Map
		mapNotes.remove(coor);
		//appelle la méthode de AdminBeanRemote qui se charge de supprimer la note de la base de données
		ClientAdmin.adminBeanRemote.removeNote(coor);
		//modifie l'itinéraire associé à cette note dans la liste des itinéraires
		removeRelatedNote(coor);
	}

	/**
	 * méthode qui trouve la note la plus proche des coordonées spécifiées en paramètres
	 * @param latitude
	 * @param longitude
	 * @return Note
	 */
	 ...
	 * @param newItinerary itinéraire en question
	 */
	public void addItinerary(Itinerary newItinerary) {
		//ajout de l'itinéraire à la liste
		itineraries.add(newItinerary);
		//appel de la méthode de AdminBeanRemote qui ajout l'itinéraire à la base de données
	...
	/**
	 * méthode qui met à jour un itinéraire de la liste lorsqu'une note est modifiée
	 * @param coor coordonées de l'ancienne note
	 * @param newNote nouvelle note pour la mise à jour
	 */
	public void updateRelatedItinerary(SCoordinate coor, Note newNote){
		...
		for (Itinerary itinerary:itineraries){
			//on appelle la méthode d'un itinéraire qui se charge de mettre à jour la note spécifiée par les coordonées
	...
	/**
	 * méthode qui retire la note dont les coordonnées sont spécifiées en paramètres
	 * @param coor coordonnées de la note
	 */
	public void removeRelatedNote(SCoordinate coor){
		//on parcourt l'ensemble des itinéraires de la liste
		for (Itinerary itinerary:itineraries){
			//on appelle la méthode d'un itinéraire qui se supprimer la note spécifiée par les coordonées
